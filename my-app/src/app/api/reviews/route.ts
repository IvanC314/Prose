...
export async function POST(request: NextRequest) {
    const { title, rating, desc, book, user_id } = await request.json(); // Expecting user_id in the request
    ...
    try {
        ...
        // CREATE USERREVIEW (Linking review with user)
        await UserReview.create({
            user_id: user_id, // Use the user_id passed in the request
            review_id: review._id,
        });

        // CREATE BOOKREVIEW (Linking review with book)
