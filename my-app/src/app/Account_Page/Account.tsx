...
//     const {logout } = useAuth(); // Get auth state and logout function
  ...
  useEffect(() => {
    const delayRedirect = setTimeout(() => {
      if (!isLoggedIn || !user_id) {
        // Redirect to login page if not logged in or user_id is null
        alert("You must be logged in to see account. Redirecting to login page");
        router.push("/Login_Page");
      }
    }, 1000); // Delay of 1 second (adjust as needed)

    return () => clearTimeout(delayRedirect); // Cleanup the timeout on component unmount
